...
var ServerInformation = {
	POIDATA_SERVER: "http://goer-project.tk/api/event.php?tag=get_events_for_AR",
	//POIDATA_SERVER: "https://example.wikitude.com/GetSamplePois/",
	POIDATA_SERVER_ARG_USERID: "user_id",
...
var World = {
	...
	loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
		...
		for (var currentPlaceNr = 0; currentPlaceNr < poiData.length; currentPlaceNr++) {
			var singlePoi = {
				...
				"host": poiData[currentPlaceNr].host,
				"txtlocat": poiData[currentPlaceNr].location,
				"start": poiData[currentPlaceNr].start_time,
				"end": poiData[currentPlaceNr].end_time,
	...
	onMarkerSelected: function onMarkerSelectedFn(marker) {
		...
		// show panel
		//$("#model_poidetail").modal('toggle');
		//$("#panel-poidetail").panel("open", 123);

		//$(".ui-panel-dismiss").unbind("mousedown");

		$('#model_poidetail').on('hidden.bs.modal', function () {
		   World.currentMarker.setDeselected(World.currentMarker);
		});

		World.onDetailButtonClicked();
	...
	updateRangeValues: function updateRangeValuesFn() {
		...
		$("#panel-distance-places").html((placesInRange != 1) ? (placesInRange + " Events") : (placesInRange + " Event"));
	...
	showRange: function showRangeFn() {
		if (World.markerList.length > 0) {
			...
			$("#panel-distance").collapse('toggle');
			//alert("Radar Clicked");
		} else {
	...
	*/
	...
	requestDataFromServer: function requestDataFromServerFn(lat, lon) {
		...
		var serverUrl = ServerInformation.POIDATA_SERVER + "&" + ServerInformation.POIDATA_SERVER_ARG_USERID + "=" + user_id ;
	...
	onDetailButtonClicked: function onDetailButtonClickedFn() {
